...
public Action TimerAnnounce(Handle timer, any client)
{
	if ((client = GetClientOfUserId(client)))
	{
		if (IsClientInGame(client) && GetClientTeam(client) != 3) {
			PrintToChat(client,"\x04[提示]\x05如果卡住时可在聊天窗口输入\x03!cs或\x03!tp\x05可传送到幸存者队伍中.");
		}
	}
...
public Action DelayTeleport(Handle timer, any client) {
	...
	if (iTarget != -1 && GetClientTeam(client) == 2) {
		...
		PrintToChat(client, "\x04[提示]\x05你已传送到幸存者队伍身边,传送次数还剩\x03%d\x05次.", G_TransmitsLimit - G_Player[client]);
	}
	else
		PrintToChat(client, "\x04[提示]\x05传送功能只限于幸存者使用.");
